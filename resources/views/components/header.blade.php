<header class="topbar">
    ...
    <div class="topbar-right">
            ...
            <div class="nav-actions" style="display: flex; gap: 16px; align-items: center; margin-right: 16px;">
                @if(isset($backUrl))
                    @if(isset($backOnclick))
                        <button class="nav-link" style="border:none; cursor:pointer;" data-action="back"
                            onclick="{{ $backOnclick }}">
                    ...
                    <div style="display: flex; gap: 16px; align-items: center;">
                        @foreach($actions as $action)
                            ...
                            @if(isset($action['onclick']))
                                <button type="button" class="{{ $action['class'] ?? '' }}" style="{{ $btnStyle }}"
                                    data-action="{{ $action['data-action'] ?? $action['onclick'] }}"
                                    onmouseover="this.style.border-color='var(--teal)'; this.style.color='var(--teal)';"
                                    onmouseout="this.style.border-color='var(--border)'; this.style.color='var(--dark)';">
                                    @if(isset($action['icon'])) <i class="{{ $action['icon'] }}" style="margin-right:6px;"></i> @endif
                                    {{ $action['text'] }}
                                </button>
                            @else
                                <a href="{{ $action['url'] }}" class="{{ $action['class'] ?? '' }}" style="{{ $btnStyle }}"
                                    @if(isset($action['data-action'])) data-action="{{ $action['data-action'] }}" @endif
        ...
        @if(isset($secondaryAction))
            <a href="{{ $secondaryAction['url'] }}" class="secondary-nav-link" title="{{ $secondaryAction['text'] }}"
                style="margin-right: 8px;">
                @if(isset($secondaryAction['icon'])) <i class="{{ $secondaryAction['icon'] }}"
                style="margin-right:6px;"></i> @endif
                {{ $secondaryAction['text'] }}
            </a>
        @endif

        @if(isset($tutorialOnclick))
            <a href="javascript:void(0)" onclick="{{ $tutorialOnclick }}" class="btn-help" title="Ayuda / Tutorial"
                style="margin-right: 8px;">
        ...
        @auth
            ...
            <div class="notifications-dropdown" style="position: relative; margin-right: 8px;">
                <button id="notiTrigger" class="btn-help" title="Notificaciones" style="position: relative;">
                    ...
                    <span id="notiBadge"
                        style="display: none; position: absolute; top: -5px; right: -5px; background: #c0392b; color: white; border-radius: 50%; width: 16px; height: 16px; font-size: 10px; font-weight: 700; align-items: center; justify-content: center;">0</span>
                </button>
                <div id="notiMenu" class="dropdown-menu-content"
                    style="display: none; position: absolute; top: calc(100% + 10px); right: 0; background: white; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); width: 280px; overflow: hidden; z-index: 1000; border: 1px solid #e2e8ef;">
                    <div
                        style="padding: 12px 16px; border-bottom: 1px solid #e2e8ef; display: flex; justify-content: space-between; align-items: center;">
                        <span
                            style="font-size: 13px; font-weight: 700; color: var(--dark); text-transform: uppercase; letter-spacing: 0.5px;">Notificaciones</span>
                        <button onclick="markNotificationsAsRead()"
                            style="border: none; background: transparent; color: var(--teal); font-size: 11px; font-weight: 600; cursor: pointer;">Marcar
                            como leídas</button>
                    </div>
                    <div id="notiList" style="max-height: 320px; overflow-y: auto;">
                        <!-- Notifications will be loaded here -->
                        <div style="padding: 20px; text-align: center; color: var(--gray2); font-size: 12px;">Cargando...
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="user-profile-dropdown" style="position: relative;">
                <div class="ubadge" id="profileTrigger"
                    style="cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 8px;">
                    <div class="avatar" id="navAvatar" style="overflow: hidden;">
                        ...
                        @else
                                    {{ collect(explode(' ', auth()->user()->name))->map(function ($word) {
                            return strtoupper(substr($word, 0, 1)); })->take(2)->join('') }}
                        @endif
                    </div>
                ...
                <div id="profileMenu" class="dropdown-menu-content"
                    ...
                    <form method="POST" action="{{ route('logout') }}" id="logout-form" style="margin: 0;">
                        ...
                        <button type="submit" class="dropdown-item"
                            style="width: 100%; border: none; background: transparent; display: flex; align-items: center; gap: 10px; padding: 12px 16px; color: #c0392b; cursor: pointer; text-align: left; font-size: 13px; font-weight: 500; transition: background 0.2s; font-family: 'Barlow', sans-serif;">
...
<style>
    ...
    .nav-link.active {
        color: var(--teal);
        background: rgba(26, 154, 138, 0.05);
    }
    ...
    .driver-popover-btn:hover {
        background: var(--accent-dark, #115e57) !important;
    }
